...
from exchange_monitor.__main__ import main
...
def test_main_routes_api_command() -> None:
    with patch("exchange_monitor.__main__._run_api") as run_api:
...
def test_main_routes_worker_command() -> None:
    with (
        patch("exchange_monitor.__main__.asyncio.run") as run_loop,
        patch("exchange_monitor.__main__._run_worker") as run_worker_factory,
    ):
...
def test_main_routes_migrate_command() -> None:
    with patch("exchange_monitor.__main__._run_migrations") as run_mig:
...
def test_main_routes_collect_command() -> None:
    with patch("exchange_monitor.__main__._run_collect") as run_collect:
